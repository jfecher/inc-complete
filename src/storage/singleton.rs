...
impl<K: OutputType> Default for SingletonStorage<K> {
    fn default() -> Self {
        Self { cell: None, value: None, key: None }
    }
}

impl<K> StorageFor<K> for SingletonStorage<K> where
    K: OutputType,
    K::Output: Eq
{
    ...
    fn insert_new_cell(&mut self, cell: Cell, _: K) {
        assert!(self.cell.is_none(), "Overwriting previous singleton value - are you using SingleStorage<{}> with a non-singleton type?", std::any::type_name::<K>());
        self.cell = Some(cell);
    }
...
impl<K> serde::Serialize for SingletonStorage<K> where
...
impl<'de, K> serde::Deserialize<'de> for SingletonStorage<K> where 
