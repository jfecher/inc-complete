...
#[derive(Clone)]
pub struct BTreeMapStorage<K: Ord + OutputType> {
    key_to_cell: BTreeMap<K, Cell>,
    cell_to_key: BTreeMap<Cell, (K, Option<K::Output>)>,
}

impl<K: Ord + OutputType> Default for BTreeMapStorage<K> {
    fn default() -> Self {
        Self { key_to_cell: Default::default(), cell_to_key: Default::default() }
    }
}

impl<K> StorageFor<K> for BTreeMapStorage<K> where
...
impl<K> serde::Serialize for BTreeMapStorage<K> where
    K: serde::Serialize + Ord + OutputType,
...
impl<'de, K> serde::Deserialize<'de> for BTreeMapStorage<K> where
    K: serde::Deserialize<'de> + Ord + OutputType + Clone,
    K::Output: serde::Deserialize<'de>,
{
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where D: serde::Deserializer<'de>
    {
        let cell_to_key: BTreeMap<Cell, (K, Option<K::Output>)> = serde::Deserialize::deserialize(deserializer)?;
        let key_to_cell = cell_to_key.iter().map(|(cell, (key, _))| (key.clone(), *cell)).collect();
        Ok(BTreeMapStorage { cell_to_key, key_to_cell })
    }
}
