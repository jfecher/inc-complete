...
impl<K> StorageFor<K> for HashMapStorage<K> where
...
impl<K> serde::Serialize for HashMapStorage<K> where
...
impl<'de, K> serde::Deserialize<'de> for HashMapStorage<K> where
    ...
    K::Output: serde::Deserialize<'de>,
{
    ...
    {
        let cell_to_key: HashMap<Cell, (K, Option<K::Output>)> = serde::Deserialize::deserialize(deserializer)?;
