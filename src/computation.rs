...
pub trait Computation: 'static + Sized + Clone {
    ...
    fn get_storage<Concrete: Computation + 'static>(computation_id: u32, container: &Self::Storage) -> &Concrete::Storage {
        ...
        assert_eq!(TypeId::of::<Concrete::Storage>(), TypeId::of::<Self::Storage>(),
    ...
    fn get_storage_mut<Concrete: Computation + 'static>(computation_id: u32, container: &mut Self::Storage) -> &mut Concrete::Storage {
        assert_eq!(computation_id, 0, "Type dispatch failed for get_storage_mut container");
    ...
    fn output_is_unset<FullComputation: Computation>(cell: Cell, computation_id: u32, original_computation_id: u32, db: &Db<FullComputation>) -> bool {
        assert_eq!(computation_id, 0, "Type dispatch failed for output_is_unset");
              ...
              Self::Output: Eq,
    {
        ...
        let container = FullComputation::get_storage_mut::<Self>(original_computation_id, db.storage_mut());
        let function = Self::get_function_and_output(cell, container).0.clone();
        let output = function.run(&mut db.handle(cell));
        FullComputation::dispatch_update_output::<Self, FullComputation>(cell, original_computation_id, original_computation_id, output, db)
    }
    ...
    fn dispatch_update_output<Concrete, FullComputation>(cell: Cell, computation_id: u32, original_computation_id: u32, output: Concrete::Output, db: &mut Db<FullComputation>) -> bool
        where Concrete: Computation,
              ...
              Self::Output: Eq,
    {
        assert_eq!(computation_id, 0, "Type dispatch failed for dispatch_update_output");
...
impl<T: OutputTypeForInput + 'static> Computation for Input<T> {
    ...
    fn run(&self, _: &mut DbHandle<impl Computation>) -> Self::Output {
        panic!("Input `{}` queried before `db.update_input(..)` called",
            std::any::type_name::<T>())
    }
