...
fn parse_type_attribute(
    ...
    match &name_value.value {
        // Handle direct types/paths
        expr => {
            // Convert the expression to tokens directly
            Ok(quote::quote! { #expr })
        }
    }
}
...
pub fn derive_storage(input: TokenStream) -> TokenStream {
    ...
    for field in fields {
        ...
        // Check if field has #[inc_complete(skip)] attribute
        ...
        if let Some(computation_type) = extract_generic_type(field_type, "Storage") {
            field_mappings.push(quote! { #field_name: #computation_type });
        } else {
            return syn::Error::new(
                field.span(),
                "Field must be a storage type like SingletonStorage<T> or HashMapStorage<T>, or use #[inc_complete(skip)] to exclude it",
            )
            .to_compile_error()
            .into();
        }
...
/// Usage:
/// ```rust
/// // Assumes ComputedValue struct is already defined
/// #[intermediate(id = 1)]
/// fn compute_double(_computed: &ComputedValue, db: &DbHandle<MyStorage>) -> i32 {
///     db.get(InputValue) * 2
/// }
/// ```
///
...
/// - **Computation type** from the first parameter (`ComputedValue` from `&ComputedValue`)
/// - **Storage type** from the second parameter (`MyStorage` from `&DbHandle<MyStorage>`)
/// - **Function name** automatically (`compute_double`)
///
/// This makes it extremely ergonomic - you only need to specify the ID!
...
pub fn intermediate(args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    match process_intermediate_function(args, input_fn, false) {
        Ok(tokens) => tokens.into(),
        Err(err) => err.to_compile_error().into(),
    }
}

/// Procedural macro for defining intermediate computations with automatic struct generation
///
/// Usage:
/// ```rust
/// #[intermediate_with_struct(id = 1)]
/// fn compute_double(_computed: &ComputedValue, db: &DbHandle<MyStorage>) -> i32 {
///     db.get(InputValue) * 2
/// }
/// ```
///
/// This generates both an empty `#[derive(Clone)] struct ComputedValue;` and a call to
/// `define_intermediate!` using the extracted information.
///
/// Required attributes:
/// - `id`: The computation ID (integer)
///
...
/// - **Function name** automatically (`compute_double`)
#[proc_macro_attribute]
pub fn intermediate_with_struct(args: TokenStream, input: TokenStream) -> TokenStream {
    let args = parse_macro_input!(args with syn::punctuated::Punctuated::<syn::Meta, syn::Token![,]>::parse_terminated);
    let input_fn = parse_macro_input!(input as syn::ItemFn);

    match process_intermediate_function(args, input_fn, true) {
        Ok(tokens) => tokens.into(),
        Err(err) => err.to_compile_error().into(),
    }
}
...
fn process_intermediate_function(
    ...
    let mut expanded = if assume_changed {
        quote! {
            #input_fn
    ...
    if generate_struct {
        let with_struct = quote! {
            #[derive(Clone)]
            struct #computation_type;
        };
        expanded.extend(with_struct);
    }
